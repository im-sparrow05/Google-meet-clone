...
function signIn(){
  ...
  let params = {
    "client_id": "443511203531-lsa6jhtsq3st739jgk46svpof2qkotg5.apps.googleusercontent.com",
    "redirect_uri": "http://127.0.0.1:3000/action.html",
    "response_type": "token",
    "scope": "https://www.googleapis.com.auth/userinfo.profile",
    "include_granted_scopes": 'true',
    'state': 'pass_hrough_value'
  }
...
function toggleWhiteboard() {
  ...
  if (isWhiteboardVisible) {
    ...
    toggleWhiteboardBtn.innerText = 'Hide Whiteboard';
  } else {
    whiteboardContainer.style.display = 'none';
    toggleWhiteboardBtn.innerText = 'Whiteboard';
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function erase(e) {
    ...
    if(isDrawing) {
      ...
      context.lineTo(e.offsetX, e.offsetY);
      const eraseSize = 30;
      context.clearRect( e.offsetX- eraseSize / 2,  e.offsetY- eraseSize / 2, eraseSize, eraseSize);
      [lastX, lastY] = [e.offsetX, e.offsetY];
  ...
  function sketch(x, y) {
    ...
    context.beginPath();
    context.arc(x, y, sketchSize / 2, 0, 2 * Math.PI);
    context.fill();
  }
  ...
  const drawBtn = document.getElementById('drawBtn');
  drawBtn.addEventListener('mousemove', toggleDrawing);
  drawBtn.addEventListener('mouseup', () => isDrawing = false);
  drawBtn.addEventListener('mouseout', () => isDrawing = false);
  ...
  const sketchBtn = document.getElementById('sketchBtn');
  sketchBtn.addEventListener('click', toggleSketching);
...
function setBackgroundColor(color) {
  const whiteboardContainer = document.getElementById('whiteboardContainer');
  whiteboardContainer.style.backgroundColor = color;
}